...
foreach my $key (keys(%{$genomes})) {
	print $key."\n";
	$gids->{$key} = $genomes->{$key};
	$gids->{$key} =~ s/\s/_/g;
	#Loading FASTA object
	my $object = {
		id => $gids->{$key}."_".$key.".contigs",
		name => $genomes->{$key}." ".$key,
		source_id => $key.".contigs",
		source => "HotLake SFA",
		type => "Organism",
		contigs => [],
		md5 => undef
	};
	my $filename = $directory.$key.".fasta";
	...
	my $md5 = $object->{md5};
	$object = {
		id => $gids->{$key}."_".$key.".genome",
		scientific_name => $genomes->{$key},
		domain => "Bacteria",
		genetic_code => 11,
		dna_size => $length,
		...
		source => "HotLake SFA",
		source_id => $gids->{$key}."_".$key.".genome",
		md5 => $object->{md5},
		taxonomy => "Unknown",
		gc_content => $gc,
		complete => 1,
