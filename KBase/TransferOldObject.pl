...
my $wsderv = get_ws_client();
my $idserv = Bio::KBase::IDServer::Client->new("http://kbase.us/services/idserver");

if (!-e "/Users/chenry/workspace/WSTransfer/".$oldws."/".$oldid) {
	die "Couldn't load specified object!";
}
open( my $fh, "<", "/Users/chenry/workspace/WSTransfer/".$oldws."/".$oldid);
my $data = "";
while (my $str = <$fh>) {
	$data .= $str;
}
...
my $class = "ModelSEED::MS::".$oldtype;
my $obj = $class->new($data);
$obj->parent($oldstore);

my $genomehash = {};
if(-e "/Users/chenry/workspace/WSTransfer/GenomeAnno.txt"){
    open( my $fh, "<", "/Users/chenry/workspace/WSTransfer/GenomeAnno.txt");
    while (my $str = <$fh>) {
		chomp($str);
		my $array = [split(/\t/,$str)];
		if (defined($array->[1])) {
...
if ($method =~ m/Bio::KBase::([^:]+)::(.+)/) {
	$provenance->[0]->{service} = $1;
	$provenance->[0]->{method} = $2;
}
$newstore->user_override($owner);

if ($oldtype eq "PhenotypeSimulationSet") {
	...
	eval {
		$NewPhenoSimSet->save($destws."/".$destid);
		print "Success:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n";
	};
	if ($@) {
		print "Failed:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n" ;
		print "ERROR_MESSAGE".$@."END_ERROR_MESSAGE\n";
	}
} elsif ($oldtype eq "Model") {
	...
	for (my $i=0; $i < @{$biomasses}; $i++) {
		...
		for (my $j=0; $j < @{$biocpds}; $j++) {
			...
			my $newbiocpd = {
				...
				coefficient => $biocpd->coefficient()+0,
			};
	...
	for (my $i=0; $i < @{$modelrxns}; $i++) {
		...
		for (my $j=0; $j < @{$prots}; $j++) {
			...
			for (my $k=0; $k < @{$subunits}; $k++) {
				...
				my $name = "unknown";
				if ($subunit->role_uuid() ne "00000000-0000-0000-0000-000000000000") {
					$name = $subunit->role()->name();
				}
				my $newsubunit = {
					role => $name,
	...
	my $count = 1;
	for (my $i=0; $i < @{$gfs}; $i++) {
		my $gf = translate_gapfill($gfs->[$i],$NewModel->_reference(),$genome->_reference(),$biochem->_reference(),$destws,$destid.".gf.".$count,$NewModel->id().".gf.".$count);
		$count++;
	...
	for (my $i=0; $i < @{$ggs}; $i++) {
		my $gg = translate_gapfill($ggs->[$i],$NewModel->_reference(),$genome->_reference(),$biochem->_reference(),$destws,$destid.".gg.".$count,$NewModel->id().".gg.".$count);
		$count++;
		push(@{$data->{gapgens}},{
		    id => $gg->id(),
			gapgen_id => $gg->id(),
	...
	if (@{$NewModel->gapgens()} > 0 || @{$NewModel->gapfillings()} > 0) {
		eval {
			$NewModel->save($destws."/".$destid);
			print "Success:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."/final\n";
		};
		if ($@) {
			print "Failed:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."/final\n" ;
			print "ERROR_MESSAGE".$@."END_ERROR_MESSAGE\n";
		}
	}
} elsif ($oldtype eq "FBA") {
	my $biochem = $newstore->get_object("kbase/default");
	my $model = $newstore->get_object($obj->model_uuid());
	my $id = $model->id().".fba.".$idserv->allocate_id_range($model->id().".fba.",1);
	my $fba = translate_fba($obj,$model->_reference(),$biochem->_reference(),$destws,$destid,$id);
} elsif ($oldtype eq "ModelTemplate") {
	...
	eval {
		$NewModelTemplate->save($destws."/".$destid);
		print "Success:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n";
	};
	if ($@) {
		print "Failed:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n" ;
	...
	eval {
		$NewPhenoSet->save($destws."/".$destid);
		print "Success:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n";
	};
	if ($@) {
		print "Failed:".$destws."/".$destid."/".$obj->{"_kbaseWSMeta"}->{wsinst}."\n" ;
	...
	if (defined($biochemstruct)) {
		...
		eval {
			$NewBiochemStruct->save($destws."/".$destid.".biochemstruct");
			print "Success:".$destws."/".$destid.".biochemstruct\n";
		};
		if ($@) {
			print "Failed:".$destws."/".$destid.".biochemstruct\n";
			print "ERROR_MESSAGE".$@."END_ERROR_MESSAGE\n";
		}
	}
	my $data = {
	...
	if (defined($obj->{contigs_uuid})) {
		...
		if (@{$newdata->{contigs}} > 0) {
			...
			eval {
			    $ContigObj->save($destws."/".$destid.".contigset");
			    print "Success:".$destws."/".$destid.".contigset\n";
			};
			if ($@) {
			    print "Failed:".$destws."/".$destid.".contigset\n" ;
			    print "ERROR_MESSAGE".$@."END_ERROR_MESSAGE\n";
			}
